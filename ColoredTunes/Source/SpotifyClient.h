...
enum SpotifyClientEPlS {
	SpotifyClientEPlSStopped = 'kPSS',
	SpotifyClientEPlSPlaying = 'kPSP',
	SpotifyClientEPlSPaused = 'kPSp'
};
typedef enum SpotifyClientEPlS SpotifyClientEPlS;
...
@interface SpotifyClientApplication : SBApplication

@property (copy, readonly) SpotifyClientTrack *currentTrack;  // The current playing track.
@property NSInteger soundVolume;  // The sound output volume (0 = minimum, 100 = maximum)
@property (readonly) SpotifyClientEPlS playerState;  // Is Spotify stopped, paused, or playing?
...
// A Spotify track.
@interface SpotifyClientTrack : SBObject
...
@interface SpotifyClientApplication (StandardSuite)
