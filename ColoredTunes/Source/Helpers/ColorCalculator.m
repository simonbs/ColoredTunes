...
@implementation ColorCalculator

#pragma mark -
#pragma mark Public Methods

+ (NSColor *)colorFromXY:(CGPoint)xy andBrightness:(float)brightness forModel:(NSString*)model {
    ...
    if (!inReachOfLamps) {
        ...
        CGPoint pAB =[self getClosestPointToPoints:[[colorPoints objectAtIndex:cptRED] pointValue] point2:[[colorPoints objectAtIndex:cptGREEN] pointValue] point3:xy];
        CGPoint pAC = [self getClosestPointToPoints:[[colorPoints objectAtIndex:cptBLUE] pointValue] point2:[[colorPoints objectAtIndex:cptRED] pointValue] point3:xy];
        CGPoint pBC = [self getClosestPointToPoints:[[colorPoints objectAtIndex:cptGREEN] pointValue] point2:[[colorPoints objectAtIndex:cptBLUE] pointValue] point3:xy];
...
+ (NSArray*)colorPointsForModel:(NSString*)model {
    ...
    {
        [colorPoints addObject:[NSValue valueWithPoint:CGPointMake(0.675F, 0.322F)]];     // Red
        [colorPoints addObject:[NSValue valueWithPoint:CGPointMake(0.4091F, 0.518F)]];    // Green
        [colorPoints addObject:[NSValue valueWithPoint:CGPointMake(0.167F, 0.04F)]];      // Blue
        
    }
    else if ([model isEqualToString:@"LLC006"] /*Bol*/ || [model isEqualToString:@"LLC007"] /*Aura*/) // Living color lights
    {
        [colorPoints addObject:[NSValue valueWithPoint:CGPointMake(0.704F, 0.296F)]];     // Red
        [colorPoints addObject:[NSValue valueWithPoint:CGPointMake(0.2151F, 0.7106F)]];   // Green
        [colorPoints addObject:[NSValue valueWithPoint:CGPointMake(0.138F, 0.08F)]];      // Blue
    }
    else
    {
        // Default construct triangle wich contains all values
        [colorPoints addObject:[NSValue valueWithPoint:CGPointMake(1.0F, 0.0F)]];         // Red
        [colorPoints addObject:[NSValue valueWithPoint:CGPointMake(0.0F, 1.0F)]];         // Green
        [colorPoints addObject:[NSValue valueWithPoint:CGPointMake(0.0F, 0.0F)]];         // Blue
    }
    return colorPoints;
}

+ (void)calculateXY:(CGPoint *)xy andBrightness:(float *)brightness fromColor:(NSColor *)color forModel:(NSString*)model {
    ...
    if (!inReachOfLamps) {
        ...
        //Find the closest point on each line in the triangle.
        CGPoint pAB =[self getClosestPointToPoints:[[colorPoints objectAtIndex:cptRED] pointValue] point2:[[colorPoints objectAtIndex:cptGREEN] pointValue] point3:xyPoint];
        CGPoint pAC = [self getClosestPointToPoints:[[colorPoints objectAtIndex:cptBLUE] pointValue] point2:[[colorPoints objectAtIndex:cptRED] pointValue] point3:xyPoint];
        CGPoint pBC = [self getClosestPointToPoints:[[colorPoints objectAtIndex:cptGREEN] pointValue] point2:[[colorPoints objectAtIndex:cptBLUE] pointValue] point3:xyPoint];
...
+ (BOOL)checkPointInLampsReach:(CGPoint)p withColorPoints:(NSArray*)colorPoints {
    ...
    CGPoint red =   [[colorPoints objectAtIndex:cptRED] pointValue];
    CGPoint green = [[colorPoints objectAtIndex:cptGREEN] pointValue];
    CGPoint blue =  [[colorPoints objectAtIndex:cptBLUE] pointValue];
